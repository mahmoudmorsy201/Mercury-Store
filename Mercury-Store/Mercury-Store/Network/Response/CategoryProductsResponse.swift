...
struct Product: Codable {
    ...
    let templateSuffix: ProductJSONNull?
    let status: Status
...
struct Variant: Codable {
    ...
    let option1: String
    let option2: Option2
    let option3: ProductJSONNull?
    let createdAt, updatedAt: Date
    let taxable: Bool
    let barcode: ProductJSONNull?
    let grams: Int
    let imageID: ProductJSONNull?
    let weight: Int
...
class ProductJSONNull: Codable, Hashable {

    public static func == (lhs: ProductJSONNull, rhs: ProductJSONNull) -> Bool {
        return true
    }
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if !container.decodeNil() {
            throw DecodingError.typeMismatch(ProductJSONNull.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Wrong type for ProductJSONNull"))
        }
    }
    ...
    static func encode(to container: inout UnkeyedEncodingContainer, array: [Any]) throws {
        for value in array {
            if let value = value as? Bool {
                ...
                try container.encode(value)
            } else if value is ProductJSONNull {
                try container.encodeNil()
